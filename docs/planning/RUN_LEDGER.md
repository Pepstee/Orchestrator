...
| **Codex doormat failure (11 Jun):** a malformed cached plugin file (`~/.codex/plugins/cache/claude-cowork/.../schedule/SKILL.md`, bad YAML) crashed Codex at startup — 57 judge calls failed without the judge ever seeing the work, each masked as a transient usage limit (bare exit-1). Cache cleared by operator 22:4x. Hardening note: teach the PA to spot IDENTICAL causes repeating across tasks (deterministic-in-disguise) | next hardening slice | with the directive-race fix |
| **Judge rerouted same-family (11 Jun):** ChatGPT plan's Codex allowance exhausted until **11 Jul 2026 ~21:15** — `AGENTIC_JUDGE=claude:opus` at launch (the pre-built outage override). Anti-collusion partially degraded; mechanical gates (acceptance-by-execution, authenticity) unaffected. RESTORE: after 11 Jul, relaunch without the env var — the registry default (openai/codex) resumes | operator | first bounce after 11 Jul |
